...
from .chapter_tools import _count_words

# Import shared helper functions
from .chapter_tools import _is_valid_chapter_filename
from .chapter_tools import _split_into_paragraphs
...
def move_paragraph_before(
    ...
    chapter_path = get_chapter_path(document_name, chapter_name)
    if not chapter_path.is_file() or not is_valid_chapter_filename(chapter_name):
        return OperationStatus(
            success=False,
            message=f"Chapter '{chapter_name}' not found in document '{document_name}'.",
        )
    ...
    try:
        ...
        paragraphs = split_content_into_paragraphs(original_full_content)
...
def register_paragraph_tools(mcp_server):
    """Register all paragraph tools with the MCP server."""

    @mcp_server.tool()
    @log_mcp_call
    @auto_snapshot("replace_paragraph")
    def replace_paragraph_tool(
        document_name: str,
        chapter_name: str,
        paragraph_index: int,
        new_content: str,
        ...
        """Replace a paragraph at a specific index within a chapter."""
    ...
    @log_mcp_call
    @auto_snapshot("insert_paragraph_before")
    def insert_paragraph_before_tool(
        document_name: str,
        chapter_name: str,
        paragraph_index: int,
        new_content: str,
        ...
        """Insert a new paragraph before a specific index within a chapter."""
    ...
    def insert_paragraph_after_tool(
        ...
        """Insert a new paragraph after a specific index within a chapter."""
    ...
    def delete_paragraph_tool(
        ...
        """Delete a paragraph at a specific index within a chapter."""
    ...
    @log_mcp_call
    @auto_snapshot("append_paragraph_to_chapter")
    def append_paragraph_to_chapter_tool(
        document_name: str,
        chapter_name: str,
        content: str,
        last_known_modified: str | None = None,
        ...
        """Append a new paragraph to the end of a chapter."""
        return append_paragraph_to_chapter(
            document_name, chapter_name, content, last_known_modified, force_write
        )

    @mcp_server.tool()
    @log_mcp_call
    @auto_snapshot("move_paragraph_before")
    def move_paragraph_before_tool(
        document_name: str,
        chapter_name: str,
        paragraph_to_move_index: int,
        target_paragraph_index: int,
        ...
        """Move a paragraph to appear before another paragraph within the same chapter."""
        return move_paragraph_before(
            document_name, chapter_name, paragraph_to_move_index, target_paragraph_index
        )

    @mcp_server.tool()
    @log_mcp_call
    @auto_snapshot("move_paragraph_to_end")
    def move_paragraph_to_end_tool(
        document_name: str, chapter_name: str, paragraph_to_move_index: int
    ) -> OperationStatus:
        """Move a paragraph to the end of a chapter."""
        return move_paragraph_to_end(
            document_name, chapter_name, paragraph_to_move_index
        )
