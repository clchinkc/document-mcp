...
# Import observability for tracing (will gracefully handle if not available)
try:
    from .observability import trace_mcp_tool as _trace_mcp_tool
...
# --- Decorator for Logging MCP Calls with Metrics and Observability ---
def log_mcp_call(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        # Use observability tracing if available (provides traces + metrics + logging)
        if OBSERVABILITY_AVAILABLE and _trace_mcp_tool is not None:
            with _trace_mcp_tool(func_name):
                return _execute_with_legacy_metrics(func, func_name, args, kwargs)
        else:
            return _execute_with_legacy_metrics(func, func_name, args, kwargs)
...
def _execute_with_legacy_metrics(func, func_name, args, kwargs):
    """Execute function with legacy metrics and logging."""
