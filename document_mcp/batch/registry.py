...
class BatchOperationRegistry:
    ...
    def get_tool_function_name(self, operation_type: str) -> str | None:
...
def register_batchable_operation(operation_type: str, tool_function_name: str = None):
    ...
    def decorator(func):
        # Use global registry from separate module to avoid circular imports
        from .global_registry import get_batch_registry

        registry = get_batch_registry()
        registry.register_operation(operation_type, tool_function_name or func.__name__)
        return func
...
def execute_batch_operation(operation: BatchOperation) -> OperationResult:
    ...
    from ..logger_config import safe_operation
    from .global_registry import get_batch_registry

    # Get the tool function name
    registry = get_batch_registry()
    tool_function_name = registry.get_tool_function_name(operation.operation_type)
...
def _get_function_for_operation(operation_type: str):
    ...
    # Get the tool function name
    registry = get_batch_registry()
    tool_function_name = registry.get_tool_function_name(operation_type)
    if not tool_function_name:
        return None
