...
class BatchOperation(BaseModel):
    ...
    operation_type: str = Field(
        ..., description="Type of operation (e.g., 'create_document')"
    )
    target: dict[str, Any] = Field(
        default_factory=dict, description="Target specification"
    )
    ...
    depends_on: list[str] | None = Field(
        default=None, description="List of operation IDs this depends on"
    )
...
class BatchApplyResult(BaseModel):
    ...
    error_summary: str | None = Field(
        default=None, description="Error summary if batch failed"
    )
...
class ConflictInfo(BaseModel):
    ...
    conflict_type: str = Field(
        ..., description="Type of conflict ('same_document', 'same_chapter')"
    )
    affected_operations: list[str] = Field(
        ..., description="Operation IDs affected by this conflict"
    )
