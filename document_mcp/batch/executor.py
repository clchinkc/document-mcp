...
class BatchExecutor:
    ...
    def __init__(self):
        ...
        self._write_operations = {
            "create_document", "delete_document", "create_chapter", "delete_chapter",
            "write_chapter_content", "replace_paragraph", "insert_paragraph_before",
            "insert_paragraph_after", "delete_paragraph", "move_paragraph_before",
            "move_paragraph_to_end", "append_paragraph_to_chapter", "replace_text"
        }
        # Operations that only read content
        self._read_operations = {
            "read_content", "find_text", "get_statistics", "list_documents", 
            "list_chapters", "read_document_summary"
        }
    
    def execute_batch(self, operations: List[BatchOperation], continue_on_error: bool = False, snapshot_id: str = None) -> BatchApplyResult:
        """
                ...
                if not continue_on_error:
                    ...
                    if successful_operations > 0:
                        if snapshot_id:
                            ...
                            rollback_performed = self._perform_manual_rollback(results[:successful_operations])
                    break
        ...
        return BatchApplyResult(
            ...
            summary=f"Executed {successful_operations}/{len(operations)} operations successfully" +
                   (f" (stopped early due to failure)" if rollback_performed else ""),
            error_summary=f"Batch failed: {failed_operations} operation(s) failed" if failed_operations > 0 else None
        )
    ...
    def _perform_manual_rollback(self, successful_results: list) -> bool:
        ...
        try:
            ...
            from ..doc_tool_server import delete_document, delete_chapter
