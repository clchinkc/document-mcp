...
def create_simple_agent_response(
    summary: str, details: Optional[Any] = None, error_message: Optional[str] = None
) -> Dict[str, Any]:
    """Create a simple agent response for testing."""
    return {"summary": summary, "details": details, "error_message": error_message}
...
def create_error_response(error_msg: str) -> Dict[str, Any]:
    ...
    return create_simple_agent_response(
        summary=f"Error: {error_msg}", details=None, error_message=error_msg
    )
...
def create_document_list_response(doc_names: list) -> Dict[str, Any]:
    ...
    return create_simple_agent_response(
        summary=f"Found {len(docs)} documents", details=docs
    )


def create_chapter_content_response(
    content: str, doc_name: str, chapter_name: str
) -> Dict[str, Any]:
...
def assert_success_response(response: Dict[str, Any], context: str = "test"):
    ...
    assert (
        "success" in summary or "created" in summary or "completed" in summary
...
def assert_error_response(
    response: Dict[str, Any], expected_error: str, context: str = "test"
):
    ...
    assert (
        expected_lower in summary or expected_lower in error_msg or "error" in summary
