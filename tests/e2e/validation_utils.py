...
class DocumentSystemValidator:
    ...
    def assert_document_not_exists(self, doc_name: str) -> None:
        ...
        """
        doc_path = self.docs_root / doc_name
        assert not doc_path.exists(), f"Document directory '{doc_name}' should not exist but found at {doc_path}"
    ...
    def assert_chapter_not_exists(self, doc_name: str, chapter_name: str) -> None:
        ...
        if doc_path.exists():
            chapter_path = doc_path / chapter_name
            assert not chapter_path.exists(), f"Chapter file '{chapter_name}' should not exist but found at {chapter_path}"
    ...
    def assert_chapter_content_contains(self, doc_name: str, chapter_name: str, expected_content: str) -> None:
    ...
    def get_document_names(self) -> Set[str]:
        ...
        return {
            item.name for item in self.docs_root.iterdir()
    ...
    def get_chapter_names(self, doc_name: str) -> Set[str]:
        ...
        return {
            item.name for item in doc_path.iterdir()
