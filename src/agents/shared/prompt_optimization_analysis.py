...
from src.agents.shared.tool_descriptions import tool_manager, ToolFormat
...
def analyze_format_optimization():
    ...
    for format_name, (char_count, token_count) in formats.items():
        reduction = f"{((baseline_tokens - token_count) / baseline_tokens * 100):.1f}%" if format_name != 'full' else "baseline"
...
def analyze_agent_prompt_sizes():
    ...
    for agent_name, prompt in agents.items():
        ...
        format_used = "Compact" if agent_name == "Simple" else ("Planner" if agent_name == "Planner" else "Full")
...
def calculate_optimization_impact():
    ...
    print(f"✅ Implemented dynamic prompt generation")
    print(f"✅ Enabled format-specific optimization")
