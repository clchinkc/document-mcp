...
class PerformanceMetricsCollector:
    ...
    def collect_from_agent_result(
        ...
        Returns:
            AgentPerformanceMetrics with real data extracted from the agent result
        """
        ...
        metrics.success = (
            agent_result.output is not None and
            not getattr(agent_result, 'error_message', None)
        )
    ...
    def validate_performance_thresholds(
        ...
        if metrics.execution_time > max_execution_time:
            violations.append(f"Execution time {metrics.execution_time:.2f}s exceeds maximum {max_execution_time}s")
...
class TokenUsageAggregator:
    ...
    @staticmethod
    def _extract_usage(agent_result: Any) -> Optional[Usage]:
