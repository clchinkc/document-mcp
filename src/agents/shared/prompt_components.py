...
def build_agent_prompt(agent_type: str, tool_descriptions: str, additional_sections: Dict[str, str] = None) -> str:
    """
    ...
    if agent_type == "simple":
        prompt_parts.append("You are an assistant that manages structured local Markdown documents using MCP tools.")
        prompt_parts.append("")
        prompt_parts.append(agent_specific["constraint"])
        prompt_parts.append("")
        prompt_parts.append(agent_specific["stopping_rule"])
    elif agent_type == "react":
        prompt_parts.append("You are an assistant that uses a set of tools to manage documents. You will break down complex user requests into a sequence of steps using the ReAct (Reason, Act) pattern.")
        prompt_parts.append("")
        ...
        prompt_parts.append("You are a specialized planning agent that generates execution plans for complex document management tasks.")
        prompt_parts.append("")
        prompt_parts.append("**YOUR ROLE:** " + agent_specific["constraint"].replace("**PLANNING ROLE:** ", ""))
    ...
    prompt_parts.append("The available tools will be discovered from an MCP server named 'DocumentManagementTools'. For detailed information on how to use each tool, including its parameters and expected behavior, refer to the description of the tool itself.")
    prompt_parts.append("")
    ...
    if agent_type == "simple":
        prompt_parts.append("")
        prompt_parts.append("Follow the user's instructions carefully and to the letter without asking for clarification unless necessary for tool parameterization.")
    elif agent_type == "react":
        prompt_parts.append("")
        prompt_parts.append("Remember: Think clearly, act precisely, and observe carefully. Always prioritize summary-driven workflows to provide efficient and comprehensive document management.")
    elif agent_type == "planner":
        prompt_parts.append("")
        prompt_parts.append("Now analyze the user's request and generate a complete execution plan in JSON format.")
